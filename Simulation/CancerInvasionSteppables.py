...
class CancerInvasionSteppable(SteppableBasePy):
    ...
    def paper_mmp_system(self):
        try:
            ...
            # Bolt optimization: Cache SWIG properties in the main step loop
            # Expected impact: Eliminates redundant SWIG boundary evaluations in helper function for each cell.
            dim_x = self.dim.x
            dim_y = self.dim.y
            ...
            for cell in self.cell_list_by_type(self.CELL):
                if self.check_ecm_contact(cell, dim_x, dim_y, ecm_fiber_type, cell_field):
            ...
            fibers_to_remove = []
            for cell in self.cell_list_by_type(ecm_fiber_type):
    ...
    def check_ecm_contact(self, cell, dim_x, dim_y, ecm_fiber_type, cell_field):
        try:
            cx, cy = int(cell.xCOM), int(cell.yCOM)
    ...
    def finish(self):
        try:
            cancer_cells = list(self.cell_list_by_type(self.CELL))
...
class ChemotaxisSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        # Bolt optimization: Cache SWIG properties once per step
        # Expected impact: Eliminates redundant SWIG boundary evaluations in helper function for each cell.
        mmp_field = self.field.MMP
        dim_x = self.dim.x
        dim_y = self.dim.y
        ...
        for cell in self.cell_list_by_type(self.CELL):
            self.apply_paper_chemotaxis(cell, mmp_field, dim_x, dim_y)

    def apply_paper_chemotaxis(self, cell, mmp_field, dim_x, dim_y):
        try:
            cx, cy = int(cell.xCOM), int(cell.yCOM)
