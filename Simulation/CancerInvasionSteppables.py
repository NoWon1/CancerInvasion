...
class CancerInvasionSteppable(SteppableBasePy):
    ...
    def check_ecm_contact(self, cell):
        try:
            ...
            # Bolt: Pre-calculate min/max bounds to avoid redundant inner-loop conditionals
            min_nx, max_nx = max(0, cx - 3), min(self.dim.x, cx + 4)
            min_ny, max_ny = max(0, cy - 3), min(self.dim.y, cy + 4)
