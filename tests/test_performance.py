...
def test_paper_mmp_system_no_exception():
    ...
    steppable.check_ecm_contact = lambda cell, dim_x, dim_y, cell_field, ecm_fiber_type: True
...
def test_check_ecm_contact_no_exception():
    ...
    try:
        result = steppable.check_ecm_contact(cell, steppable.dim.x, steppable.dim.y, steppable.cell_field, steppable.ECMFIBER)
...
def test_chemotaxis_steppable_apply_no_exception():
    ...
    try:
        steppable.apply_paper_chemotaxis(cell, steppable.field.MMP, steppable.dim.x, steppable.dim.y)
    except Exception as e:
